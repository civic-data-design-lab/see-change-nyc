var dataset = d3.csv("./data/nyt_data2.csv").then(function (data) {
...
function createGrid(svg, dataset) {
  ...
  function circle() {
    ...
    circles
      .filter(d => d.category === "topicblank")
      .classed("circle", false)
    circles
      .filter(d => d.topic === "yearblank")
      .classed("circle", false)
  ...
  function groupBubble() {
    ...
    function yearpositionX(i) {
      return ((width +120) / 5) * ((i % 4) + 1) -60
    }
    function yearpositionY(i) {
      return (Math.floor(i/4)+1) * height/3 +100
    }
